...
float GPS::getLeftPosGps() {
  ...
  return (Motor_BaseLF.position(deg)+Motor_BaseLM.position(deg)) * CHASSIS_GEAR_RATIO * (WHEEL_DIAMETER * 25.4 * 3.1416) / (2.0*360); // return mm
}

float GPS::getRightPosGps() {
  // return the position of right side of base (mm from last reset position) according to encoder value
  return (Motor_BaseRF.position(deg)+Motor_BaseRM.position(deg)) * CHASSIS_GEAR_RATIO * (WHEEL_DIAMETER * 25.4 * 3.1416) / (2.0*360); // return mm
}
...
void GPS::updateGpsPos(){
  ...
  static float currForwardPos, currHeading, currPitch;
  ...
  // gpsLock.writeLock();
  float factor = cos(deg2rad(0.5*(currPitch+lastPitch)));
  float delta = currForwardPos - lastForwardPos;
  float deltaH = currHeading - lastHeading;

  if (abbs(deltaH) < 10.0 && abbs(delta)<50.0) {
    gpsPosX += factor* delta * sin(deg2rad(0.5 * (currHeading + lastHeading)));
    gpsPosY += factor* delta * cos(deg2rad(0.5 * (currHeading + lastHeading)));
...
void GPS::gpsAim(float _xGoal, float _yGoal, float kp, float ki, float kd) {
  float _targetHeading = calTheta(_xGoal - gpsPosX, _yGoal - gpsPosY);
  PIDAngleRotateAbs(_targetHeading, kp, ki, kd, 0.5);
}

void GPS::gpsAimMove(float _xGoal, float _yGoal, float distance, int _direct, int _maxPower, std::string mode) {
  float _targetHeading = calTheta(_xGoal - gpsPosX, _yGoal - gpsPosY);
  
  if (fabs(gpsHeading - _targetHeading) > 1 && mode != "continue") PIDAngleRotateAbs(_targetHeading, 4, 0.1, 40, 1.5);
  // softStartTimerForward(getCrtVel(), _maxPower*_direct, _maxPower);
  posForwardAbsWithHeadingGPS(_maxPower, 0.75*distance*_direct, _xGoal, _yGoal);
  PIDPosForwardAbs(distance*_direct);
  _targetHeading = calTheta(_xGoal - gpsPosX, _yGoal - gpsPosY);
  if (fabs(gpsHeading - _targetHeading) > 1) PIDAngleRotateAbs(_targetHeading, 1.5, 0.3, 45, 0.5);
}
